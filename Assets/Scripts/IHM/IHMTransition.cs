...
public class IHMTransition : MonoBehaviour {
    ...
    public void Transition_optionAudioFromInterview()//menu 5
    {
        IHMInterview.MaskAllNguiComponents(false);
        Time.timeScale = 0;
        SceneManager.LoadScene("6-Audio_Menu_from_interview",LoadSceneMode.Additive);
        //SceneManager.SetActiveScene(SceneManager.GetSceneByName("6-Audio_Menu_from_interview"));
    }
    public void UnloadSceneOptionAudio()
    {
        IHMInterview.MaskAllNguiComponents(true);
        Time.timeScale = 1;
        SceneManager.UnloadSceneAsync("6-Audio_Menu_from_interview");
    }

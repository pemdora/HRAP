...
#pragma warning disable 618
		...
		public void SetEmotion (string emotion, float blendTime) {
			if (!IsPlaying && ready && enabled) {
				EmotionMarker emotionMarker = new EmotionMarker(emotion, 0, 1, blendTime, 0, false, false, true, true);

				LipSyncData data = ScriptableObject.CreateInstance<LipSyncData>();
				data.length = blendTime;
				data.gestureData = new GestureMarker[0];
				data.phonemeData = new PhonemeMarker[0];
				data.emotionData = new EmotionMarker[] { emotionMarker };
				data.version = lastUsedVersion;

				bool oldkewf = keepEmotionWhenFinished;
				keepEmotionWhenFinished = true;
				onFinishedPlaying.AddListener(() => {
					keepEmotionWhenFinished = oldkewf;
				});

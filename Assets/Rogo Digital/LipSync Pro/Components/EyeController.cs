...
namespace RogoDigital {
	...
	public class EyeController : BlendSystemUser {
		...
		private Quaternion[] axisOffsets = { Quaternion.Euler(0, 90, 0), Quaternion.Euler(0, -90, 0), Quaternion.Euler(90, 0, 0), Quaternion.Euler(-90, 0, 0), Quaternion.Euler(0, 0, 0), Quaternion.Euler(0, 180, 0) };
		...
		void LateUpdate () {
			...
			if (targetEnabled && lookingControlMode != ControlMode.PoseBased && leftEyeLookAtBone != null && rightEyeLookAtBone != null) {
				...
				if (target != null) {
					...
					llta = new Vector3(Mathf.Clamp(llta.x, eyeRotationRangeX.x, eyeRotationRangeX.y), Mathf.Clamp(llta.y, eyeRotationRangeY.x, eyeRotationRangeY.y), eyeLookOffset.z);
					lrta = new Vector3(Mathf.Clamp(lrta.x, eyeRotationRangeX.x, eyeRotationRangeX.y), Mathf.Clamp(lrta.y, eyeRotationRangeY.x, eyeRotationRangeY.y), eyeLookOffset.z);

					leftTargetAngle = Quaternion.Euler(leftEyeLookAtBone.parent.TransformEulerAngle(llta));
					rightTargetAngle = Quaternion.Euler(rightEyeLookAtBone.parent.TransformEulerAngle(lrta));
				} else {
					targetWeight = 0;
				}
			} else {
			...
			if (leftEyeLookAtBone != null && rightEyeLookAtBone != null && (randomLookingEnabled || targetEnabled) || lookingControlMode == ControlMode.PoseBased) {
				if (lookingControlMode == ControlMode.Classic) {
					...
					leftEyeLookAtBone.localRotation *= axisOffsets[(int)eyeForwardAxis];
					rightEyeLookAtBone.localRotation *= axisOffsets[(int)eyeForwardAxis];
				} else if (lookingControlMode == ControlMode.PoseBased) {
					if (randomBlend.y >= 0) {
						// Looking Up Range
